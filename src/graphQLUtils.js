  ...
  switch (data.__typename) {
    ...
    case "ParticipantLeftMessage":
      return {
        ...
        eventData: {
          // Array of IDs.
          participantsRemoved: data.participants_removedata.map(function(p) {
            return p.id;
          })
        },
        ...
        logMessageData: {
          leftParticipantFbId: data.participants_removedata.map(function(p) {
            return p.id;
          })
        }
      };
    case "ParticipantsAddedMessage":
      return {
        ...
        eventData: {
          // Array of IDs.
          participantsAdded: data.participants_addedata.map(function(p) {
            return p.id;
          })
        },
        ...
        logMessageData: {
          addedParticipants: data.participants_addedata.map(function(p) {
            return p.id;
          })
        }
      };
...
function graphQLBatch(ctx, defaultFuncs, query) {
  ...
  var form = {
    queries: JSON.stringify({
      // @TODO Do we want to support multiple queries?
      o0: query
    })
  };
